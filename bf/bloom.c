...
BLOOM *bloom_create(size_t n, float p)
{
	...
	bloom->mbits = (size_t)(-n*log(p)/(log(2)*log(2)));
	bloom->len = _S_round_up(bloom->mbits)%CHAR_BIT;

	if (!(bloom->s = calloc(bloom->len,sizeof(char)))) {
		free(bloom);
		return(NULL);
	}

	bloom->hashn = (int)(0.7*(bloom->mbits)/n);
	bloom->hash = _defaultBloomHash;
...
int bloom_add(BLOOM *bloom, const char *s)
{
	...
	for (n = 0; n < bloom->hashn; n++) {
		buf[slen] = n + 1;
		SETBIT(bloom->s, (*bloom->hash)(buf)%bloom->mbits);
	}
...
int bloom_check(BLOOM *bloom, const char *s)
{
	...
	for (n = 0; n < bloom->hashn; n++) {
		buf[slen] = n + 1;
		if (!(GETBIT(bloom->s, (*bloom->hash)(buf)%bloom->mbits)))
			return(0);
	}
