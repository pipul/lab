...
Node listnodes(Tree tp, int tlab, int flab)
{
	...
	if (forest->op == LABEL + V) {
	    equatelab(forest->syms[0], findlabel(tlab + 1));
	    unlist();
	}
	list(jump(tlab + 1));
	labelnode(tlab);
	...
	*/
    ...
    break;
    case RIGHT: { //
	Tree lkid = tp->kids[0], rkid = tp->kids[1];
        if (lkid && rkid && generic(rkid->op) == ASGN
	    && (generic(lkid->op) == INDIR && lkid->kids[0] == rkid->kids[0] || (lkid->op == FIELD &&  lkid == rkid->kids[0]))) {
            assert(tlab == 0 && flab == 0);
            if (generic(lkid->op) == INDIR) {
                p = listnodes(lkid, 0, 0);
                list(p);
                listnodes(rkid, 0, 0);
            } else {
                assert(generic(lkid->kids[0]->op) == INDIR);
                list(listnodes(lkid->kids[0], 0, 0));
                p = listnodes(lkid, 0, 0);
                listnodes(rkid, 0, 0);
            }
        } else if (rkid) {
            listnodes(lkid, 0, 0);
            p = listnodes(rkid, tlab, flab);
        } else
            p = listnodes(lkid, tlab, flab);
    }
    break;
    case JUMP:  { // goto
        assert(tlab == 0 && flab == 0);
        ...
        if (tp->kids[0]->op == FIELD) {
            ...
            if (fieldsize(f) < 8*f->type->size) {
                ...
                if (q->op == CNST+I && q->u.v.i == 0 ||  q->op == CNST+U && q->u.v.u == 0)
                    ...
                    listnodes(q, 0, 0);
                    q = bittree(BOR,
				bittree(BAND, rvalue(lvalue(x)), cnsttree(unsignedtype, (unsigned long)~mask)),
				bittree(BAND, shtree(LSH, cast(q, unsignedtype),cnsttree(unsignedtype, (unsigned long)fieldright(f))), cnsttree(unsignedtype, (unsigned long)mask)));
                }
                r = listnodes(q, 0, 0);
                op = ASGN + ttob(q->type);
            } else {
...
/* printforest - print forest p on fd */
void printforest(Node p, int fd)
{
    int i;
    Node p1;
    FILE *f = fd == 1 ? stdout : stderr;
    
    for (i = 0, p1 = p->link; p1 ; p1 = p1->link, i++) {
	fprintf(f, "dag %d:\n", i);
	printdag(p, fd);
    }
}
...
static void printnode(Node p, int fd, int lev)
{
	...
	fprint(f, "#%d%S%s", id, blanks, lev, &"   "[id < 10 ? 0 : id < 100 ? 1 : 2]);
